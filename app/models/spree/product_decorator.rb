Spree::Product.class_eval do
  ...
  scope :by_serial, lambda { |profile| joins(:variants).where("spree_variants.tire_serial_id = ?", serial)}
  scope :by_innertube, lambda { |innertube| joins(:variants).where("spree_variants.tire_innertube_id = ?", innertube)}
  scope :by_gr, lambda { |ic| joins(:variants).where("spree_variants.tire_gr = ?", gr) }
  scope :by_speed, lambda { |speed| joins(:variants).where("spree_variants.tire_speed_code_id = ?", speed)}
  scope :by_rf, lambda { |fr| joins(:variants).where("spree_variants.tire_rf = ?", rf)}
  scope :by_season, lambda { |tttl| joins(:variants).where("spree_variants.tire_season = ?", season)}
end
